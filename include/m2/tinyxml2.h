...
//#ifdef BY_CMAKE_COMPILE
//#include "XPCore_global.h"
//#endif

#if defined(_WIN32) || defined(_WIN64)
#ifdef XPCORE_EXPORTS
...
/**
	...
	Generally Accept() is called on the XXMLDocument, although all nodes support visiting.
...
public:
    ...
    /// Visit a document.
    virtual bool VisitEnter(const XXMLDocument & /*doc*/) { return true; }
    /// Visit a document.
    virtual bool VisitExit(const XXMLDocument & /*doc*/) { return true; }
...
// WARNING: must match XXMLDocument::_errorNames[]
enum XMLError
{
    XML_SUCCESS = 0,
    XML_NO_ATTRIBUTE,
...
/** XMLNode is a base class for every object that is in the
	...
	be navigated. A node is always in a XXMLDocument.
	The type of a XMLNode can be queried, and it can
	be cast to its more defined type.

	A XXMLDocument allocates memory for all its Nodes.
	When the XXMLDocument gets deleted, all its Nodes
...
{
    friend class XXMLDocument;
    friend class XMLElement;

public:
    /// Get the XXMLDocument that owns this XMLNode.
    const XXMLDocument *GetDocument() const
    {
        TIXMLASSERT(_document);
        return _document;
    }
    /// Get the XXMLDocument that owns this XMLNode.
    XXMLDocument *GetDocument()
    {
        TIXMLASSERT(_document);
        return _document;
    }
    ...
    virtual XXMLDocument *ToDocument() { return 0; }
    ...
    virtual const XXMLDocument *ToDocument() const { return 0; }
    ...
    */
    ...
    /**
    	...
    	Note: if called on a XXMLDocument, this will return null.
    */
    virtual XMLNode *ShallowClone(XXMLDocument *document) const = 0;

    /**
    	Test if 2 nodes are the same, but don't test children.
    	The 2 nodes do not need to be in the same Document.

    	Note: if called on a XXMLDocument, this will return false.
    */
...
protected:
    XMLNode(XXMLDocument *);
    virtual ~XMLNode();

    virtual char *ParseDeep(char *, StrPair *);

    XXMLDocument *_document;
    XMLNode *_parent;
...
public:
    ...
    virtual XMLNode *ShallowClone(XXMLDocument *document) const;
    virtual bool ShallowEqual(const XMLNode *compare) const;

protected:
    XMLText(XXMLDocument *doc) : XMLNode(doc), _isCData(false) {}
    virtual ~XMLText() {}
...
protected:
    XMLDeclaration(XXMLDocument *doc);
...
*/
class TINYXML2_LIB XXMLDocument : public XMLNode
{
    friend class XMLElement;

public:
    /// constructor
    XXMLDocument(bool processEntities = true, Whitespace = PRESERVE_WHITESPACE);
    ~XXMLDocument();

    virtual XXMLDocument *ToDocument()
    {
        TIXMLASSERT(this == _document);
        return this;
    }
    virtual const XXMLDocument *ToDocument() const
    {
        TIXMLASSERT(this == _document);
        return this;
    }
...
private:
    XXMLDocument(const XXMLDocument &);  // not supported
    void operator=(const XXMLDocument &);// not supported
...
/**
	...
	options than the XXMLDocument::Print() method.

	It can:
	-# Print to memory.
	-# Print to a file you provide.
	-# Print XML without a XXMLDocument.
...
public:
    ...
    */
    ...
    virtual bool VisitEnter(const XXMLDocument & /*doc*/);
    virtual bool VisitExit(const XXMLDocument & /*doc*/) { return true; }
