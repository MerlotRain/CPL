...
{
public:
    GsScopedRWLock(GsRWLock &l, bool write = false);
    ~GsScopedRWLock();
...
{
public:
    GsScopedWriteRWLock(GsRWLock &l);
    ~GsScopedWriteRWLock();
};
...
public:
    ...
    /// @param milliseconds
    template<class M>
    void TryWaitUntil(M &mtx, int milliseconds)
    {
        GsScopedLock<M> ul(mtx);
        GsWaitEvent event;
        {
