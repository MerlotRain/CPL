...
{
public:
    ThreadPool(std::size_t threadSize = std::thread::hardware_concurrency(),
               std::size_t taskSize = 128)
        : _stop(false), _workerThreads(threadSize), _maxTaskSize(taskSize)
    {
        auto Func = [this]() {
            while (true)
            {
    ...
    std::size_t &maxTaskSize() { return _maxTaskSize; }

    std::size_t &threadSize() { return _threadSize; }

    template<typename Fun, typename... Args>
    auto enqueue(Fun &&_fun, Args &&...args)
            -> std::future<typename std::result_of<Fun(Args...)>::type>
    {
        ...
        auto task =
                std::make_shared<std::packaged_task<return_type()>>{std::bind(
                        std::forward<Fun>(_fun), std::forward<Args>(args)...)};
        std::future<return_type> res = task->get_future();

        std::unique_lock<std::mutex> l(this->_mutex);

        this->_condition.wait(l, [this] {
            return this->_stop || this->_queue.size() < this->_maxTaskSize;
        });
        if (this->_stop) { return res; }
