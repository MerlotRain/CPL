...
static char *__uuidToHex(const UUID &uuid, char *dst,
                         UUID::StringFormat mode = UUID::eHyphens32)
{
    switch (mode)
    {
...
static UUID __uuidFromHex(const char *src)
{
    ...
    {
        if (*src == '{') src++;
        if (__fromHex(src, d1) && *src++ == '-' && __fromHex(src, d2) &&
            *src++ == '-' && __fromHex(src, d3) && *src++ == '-' &&
            __fromHex(src, d4[0]) && __fromHex(src, d4[1]) && *src++ == '-' &&
            __fromHex(src, d4[2]) && __fromHex(src, d4[3]) &&
            __fromHex(src, d4[4]) && __fromHex(src, d4[5]) &&
            __fromHex(src, d4[6]) && __fromHex(src, d4[7]))
        {
...
UUID::UUID(unsigned int l, unsigned short w1, unsigned short w2,
...
{
    return createFromName(ns, ByteArray(baseData.c_str()),
                          CryptographicHash::eMD5, 3);
}

UUID UUID::CreateUuidV5(const UUID &ns, const String &baseData)
{
    return createFromName(ns, ByteArray(baseData.c_str()),
                          CryptographicHash::eSHA1, 5);
}
...
{
    if (bytes.IsEmpty() || bytes.BufferLength() != 16) return UUID();
    ...
    const unsigned char *data =
            reinterpret_cast<const unsigned char *>(bytes.BufferHead());
